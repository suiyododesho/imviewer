"""Generate derived JavaScript files from site/structure.json."""

import argparse
import json
import os
import re
import sys
import unicodedata
from urllib.parse import unquote
from PIL import Image, ImageOps

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from history_manager import parse_history, write_history  # noqa: E402

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
SITE_DIR = os.path.join(ROOT, "site")
CONTENTS_DIR = os.path.join(SITE_DIR, "contents")
STRUCTURE_PATH = os.path.join(SITE_DIR, "structure.json")
JS_DIR = os.path.join(SITE_DIR, "js")
STRUCTURE_JS_PATH = os.path.join(JS_DIR, "structure.js")
GALLERY_PAGES_PATH = os.path.join(JS_DIR, "gallery-pages.js")
HISTORY_PATH = os.path.join(SITE_DIR, "history.txt")
PHOTO_DIR = os.path.join(CONTENTS_DIR, "photo")


NEXT_RE = re.compile(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>\s*<img[^>]*alt=["\']次へ["\']', re.IGNORECASE | re.DOTALL)
IMG_RE = re.compile(r'<img[^>]+src=["\']([^"\']+\.(?:jpg|jpeg|png|webp|gif))["\']', re.IGNORECASE)
IMAGE_EXTENSIONS = ('.jpg', '.jpeg', '.png', '.webp', '.gif', '.bmp', '.avif')
VIDEO_EXTENSIONS = ('.avi', '.mpg', '.mpeg', '.mp4', '.mkv', '.wmv')
GALLERY_THUMB_SIZE = (220, 220)


def _resample_lanczos():
    try:
        return Image.Resampling.LANCZOS
    except AttributeError:
        return Image.LANCZOS


def _display_width(text):
    """Return terminal column width of text, counting wide (CJK) chars as 2."""
    width = 0
    for ch in text:
        eaw = unicodedata.east_asian_width(ch)
        width += 2 if eaw in ('W', 'F') else 1
    return width


class ProgressLine:
    def __init__(self):
        self.enabled = bool(getattr(sys.stdout, 'isatty', lambda: False)())
        self._last_width = 0

    def update(self, text):
        if not self.enabled:
            return

        message = str(text)
        width = _display_width(message)
        pad = max(0, self._last_width - width)
        sys.stdout.write('\r' + message + (' ' * pad))
        sys.stdout.flush()
        self._last_width = width

    def finish(self):
        if not self.enabled:
            return
        sys.stdout.write('\n')
        sys.stdout.flush()
        self._last_len = 0


def norm_rel(path):
    return path.replace('\\', '/').lstrip('./')


def resolve_rel(base_file_rel, target_rel):
    base_dir = os.path.dirname(base_file_rel)
    joined = os.path.normpath(os.path.join(base_dir, unquote(target_rel)))
    return norm_rel(joined)


def read_text(path):
    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        return f.read()


def parse_page(page_rel, gallery_root_rel=None):
    page_abs = os.path.join(CONTENTS_DIR, page_rel)
    if not os.path.isfile(page_abs):
        return None, None

    text = read_text(page_abs)

    img_candidates = [m.group(1) for m in IMG_RE.finditer(text)]
    img_rel = None
    gallery_root_prefix = None
    if gallery_root_rel:
        gallery_root_prefix = norm_rel(gallery_root_rel).rstrip('/') + '/'

    # Prefer real image files located under the same gallery root.
    # This avoids accidentally selecting navigation assets such as works/next.jpg.
    for cand in img_candidates:
        if cand.lower().startswith('data:'):
            continue
        resolved = resolve_rel(page_rel, cand)
        if gallery_root_prefix and not resolved.startswith(gallery_root_prefix):
            continue
        if not os.path.isfile(os.path.join(CONTENTS_DIR, resolved)):
            continue
        img_rel = resolved
        break

    # Fallback for unusual legacy pages: use the first existing non-data image.
    if img_rel is None:
        for cand in img_candidates:
            if cand.lower().startswith('data:'):
                continue
            resolved = resolve_rel(page_rel, cand)
            if os.path.isfile(os.path.join(CONTENTS_DIR, resolved)):
                img_rel = resolved
                break

    next_rel = None
    m = NEXT_RE.search(text)
    if m:
        next_href = m.group(1)
        if next_href and next_href != '#':
            next_rel = resolve_rel(page_rel, next_href)

    return img_rel, next_rel


def gather_gallery_pages(start_rel):
    pages = []
    seen = set()
    current = norm_rel(start_rel)
    gallery_root_rel = os.path.dirname(current)

    for _ in range(2000):
        if current in seen:
            break
        seen.add(current)

        img_rel, next_rel = parse_page(current, gallery_root_rel)
        if img_rel:
            pages.append({
                'type': 'image',
                'image': f"contents/{img_rel}",
                'html': f"contents/{current}",
            })

        if not next_rel:
            break

        current = next_rel

    return pages


def gather_media_from_gallery_tree(start_rel):
    page_abs = os.path.join(CONTENTS_DIR, start_rel)
    if not os.path.isfile(page_abs):
        return []

    gallery_root_abs = os.path.dirname(page_abs)
    image_pages = []
    video_pages = []
    video_index = 0

    for root, dirs, files in os.walk(gallery_root_abs):
        # Exclude prebuilt thumbnail directory (src/thumbnail) from media collection.
        dirs[:] = sorted(
            (d for d in dirs if not (d.lower() == 'thumbnail' and os.path.basename(root).lower() == 'src')),
            key=str.lower,
        )

        for name in files:
            lower_name = name.lower()
            file_abs = os.path.join(root, name)
            file_rel = norm_rel(os.path.relpath(file_abs, CONTENTS_DIR))

            if lower_name.endswith(IMAGE_EXTENSIONS):
                image_pages.append({
                    'type': 'image',
                    'image': f"contents/{file_rel}",
                    # Keep the original gallery HTML as the open-target for scanned images.
                    'html': f"contents/{start_rel}",
                })
                continue

            if lower_name.endswith(VIDEO_EXTENSIONS):
                video_index += 1
                video_pages.append({
                    'type': 'video',
                    'video': f"contents/{file_rel}",
                    'html': f"contents/{start_rel}",
                    'thumbNumber': video_index,
                    'label': os.path.splitext(name)[0],
                    'ext': os.path.splitext(name)[1].lstrip('.').lower(),
                })

    return image_pages + video_pages


def dedupe_pages(pages):
    seen_media = set()
    unique = []

    for page in pages:
        media_key = page.get('image') or page.get('video') or page.get('html')
        if not media_key or media_key in seen_media:
            continue
        seen_media.add(media_key)
        unique.append(page)

    return unique


def media_uri_to_rel(uri):
    value = str(uri or '')
    if not value:
        return ''
    if value.startswith('contents/'):
        return norm_rel(value[len('contents/'):])
    return norm_rel(value)


def ensure_gallery_prebuilt_thumbnails(gallery_path, pages):
    gallery_root_rel = os.path.dirname(norm_rel(gallery_path))
    thumb_dir_rel = norm_rel(os.path.join(gallery_root_rel, 'src', 'thumbnail'))
    thumb_dir_abs = os.path.join(CONTENTS_DIR, thumb_dir_rel)
    os.makedirs(thumb_dir_abs, exist_ok=True)

    resample = _resample_lanczos()
    generated = 0
    reused = 0

    thumb_index = 0
    for page in pages:
        if page.get('type') != 'image':
            continue

        src_rel = media_uri_to_rel(page.get('image'))
        if not src_rel:
            continue

        src_abs = os.path.join(CONTENTS_DIR, src_rel)
        if not os.path.isfile(src_abs):
            continue

        thumb_index += 1
        thumb_name = f'{thumb_index:03d}.jpg'
        thumb_rel = norm_rel(os.path.join(thumb_dir_rel, thumb_name))
        thumb_abs = os.path.join(CONTENTS_DIR, thumb_rel)

        needs_build = True
        if os.path.isfile(thumb_abs):
            try:
                needs_build = os.path.getmtime(thumb_abs) < os.path.getmtime(src_abs)
            except OSError:
                needs_build = True

        if needs_build:
            try:
                with Image.open(src_abs) as im:
                    prepared = ImageOps.exif_transpose(im).convert('RGB')
                    fitted = ImageOps.fit(prepared, GALLERY_THUMB_SIZE, method=resample, centering=(0.5, 0.5))
                    fitted.save(thumb_abs, format='JPEG', quality=84, optimize=True)
                generated += 1
            except Exception as exc:
                print(f'WARN: failed to build gallery thumbnail: {src_rel} -> {thumb_rel} ({exc})')
                page['thumbnail'] = page.get('image')
                continue
        else:
            reused += 1

        page['thumbnail'] = f'contents/{thumb_rel}'

    return generated, reused


def iter_gallery_paths(structure_obj):
    for _, project_data in structure_obj.items():
        if not isinstance(project_data, dict):
            continue
        for key, person_data in project_data.items():
            if key in ('label', 'banner', 'series'):
                continue
            if not isinstance(person_data, dict):
                continue
            galleries = person_data.get('galleries', [])
            for gallery in galleries:
                path = gallery.get('path')
                if path:
                    yield norm_rel(path)


def write_structure_js(structure):
    os.makedirs(JS_DIR, exist_ok=True)
    with open(STRUCTURE_JS_PATH, 'w', encoding='utf-8', newline='\n') as out:
        out.write('/**\n')
        out.write(' * Auto-generated from site/structure.json.\n')
        out.write(' * Do not edit manually. Rebuild using tools/build_gallery_pages_map.py\n')
        out.write(' */\n\n')
        out.write('window.siteStructure = ')
        json.dump(structure, out, ensure_ascii=False, indent=2)
        out.write(';\n')


def write_gallery_pages_js(result):
    os.makedirs(JS_DIR, exist_ok=True)
    with open(GALLERY_PAGES_PATH, 'w', encoding='utf-8', newline='\n') as out:
        out.write('/**\n')
        out.write(' * Auto-generated gallery pages map from site/structure.json.\n')
        out.write(' * Do not edit manually. Rebuild using tools/build_gallery_pages_map.py\n')
        out.write(' */\n')
        out.write('window.galleryPagesMap = ')
        json.dump(result, out, ensure_ascii=False, separators=(',', ':'))
        out.write(';\n')


def get_all_photo_leaf_dirs(photo_dir):
    """Scan photo/ and return set of 'photo/<series>/<person>' paths (2-level)."""
    paths = set()
    if not os.path.isdir(photo_dir):
        return paths
    for series in sorted(os.listdir(photo_dir)):
        series_abs = os.path.join(photo_dir, series)
        if not os.path.isdir(series_abs):
            continue
        for person in sorted(os.listdir(series_abs)):
            person_abs = os.path.join(series_abs, person)
            if not os.path.isdir(person_abs):
                continue
            paths.add(f'photo/{series}/{person}')
    return paths


def detect_new_photo_dirs(photo_dir, history_data):
    """Return sorted photo leaf dirs that are not yet recorded in history."""
    current_paths = get_all_photo_leaf_dirs(photo_dir)
    known_paths = history_data.all_known_dirs()
    return sorted(current_paths - known_paths)


def record_photo_diff(photo_dir, history_path):
    """Detect new photo leaf dirs and append them to the next: block in history.txt."""
    data = parse_history(history_path)
    new_paths = detect_new_photo_dirs(photo_dir, data)

    if not new_paths:
        print('No new photo directories detected.')
        return

    data.next_dirs.extend(new_paths)
    write_history(history_path, data)

    print(f'Recorded {len(new_paths)} new photo directories to history.txt (next: block):')
    for p in new_paths:
        print(f'  {p}')


def _normalize_history_dir(path):
    normalized = norm_rel(str(path or '').strip())
    if normalized.startswith('contents/'):
        normalized = normalized[len('contents/'):]
    return normalized.rstrip('/')


def _is_same_or_child(path, parent):
    return path == parent or path.startswith(parent + '/')


def select_gallery_paths_for_diff(gallery_paths, target_dirs):
    targets = [
        _normalize_history_dir(item)
        for item in (target_dirs or [])
        if _normalize_history_dir(item)
    ]
    if not targets:
        return []

    selected = []
    for gallery_path in gallery_paths:
        gallery_root = norm_rel(os.path.dirname(gallery_path)).rstrip('/')
        for target in targets:
            # Match direct directory entries as well as parent/child directory relations.
            if _is_same_or_child(gallery_root, target) or _is_same_or_child(target, gallery_root):
                selected.append(gallery_path)
                break
    return selected


def load_existing_gallery_pages_map(path):
    if not os.path.isfile(path):
        return None
    text = read_text(path)
    match = re.search(r'window\.galleryPagesMap\s*=\s*(\{.*\})\s*;', text, re.DOTALL)
    if not match:
        return None
    try:
        data = json.loads(match.group(1))
    except json.JSONDecodeError:
        return None
    return data if isinstance(data, dict) else None


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description='Generate structure.js and gallery-pages.js from structure.json')
    parser.add_argument(
        '--diff',
        action='store_true',
        help='Rebuild only galleries matching history.txt next:{dirs,force_dirs} entries',
    )
    return parser.parse_args(argv)


def main():
    args = parse_args()

    with open(STRUCTURE_PATH, 'r', encoding='utf-8') as f:
        structure = json.load(f)

    all_gallery_paths = list(iter_gallery_paths(structure))
    target_gallery_paths = list(all_gallery_paths)
    result = {}
    incremental_mode = False

    if args.diff:
        history_data = parse_history(HISTORY_PATH)
        history_targets = list(history_data.next_dirs) + list(history_data.next_force_dirs)
        newly_detected_dirs = detect_new_photo_dirs(PHOTO_DIR, history_data)
        if newly_detected_dirs:
            history_targets.extend(newly_detected_dirs)
            print(
                f'Diff mode: found {len(newly_detected_dirs)} unrecorded photo directories. '
                'Including them in this run and appending to history.txt at the end.'
            )
        selected = select_gallery_paths_for_diff(all_gallery_paths, history_targets)
        existing_map = load_existing_gallery_pages_map(GALLERY_PAGES_PATH)

        if existing_map is None:
            print('WARN: --diff requested but existing js/gallery-pages.js is missing or invalid. Running full rebuild.')
        elif not selected:
            valid_galleries = set(all_gallery_paths)
            result = {
                key: value
                for key, value in existing_map.items()
                if key in valid_galleries
            }
            target_gallery_paths = []
            incremental_mode = True
            print('No galleries matched history next entries. Keeping existing gallery-pages.js entries.')
        else:
            valid_galleries = set(all_gallery_paths)
            result = {
                key: value
                for key, value in existing_map.items()
                if key in valid_galleries
            }
            target_gallery_paths = selected
            incremental_mode = True
            print(
                f'Diff mode: rebuilding {len(target_gallery_paths)} / {len(all_gallery_paths)} galleries '
                f'from history next entries.'
            )

    thumb_generated = 0
    thumb_reused = 0
    progress = ProgressLine()
    total_galleries = len(target_gallery_paths)

    for index, gallery_path in enumerate(target_gallery_paths, 1):
        gallery_root = norm_rel(os.path.dirname(gallery_path))
        percent = int((index * 100) / total_galleries) if total_galleries else 100
        progress.update(
            f'[{index}/{total_galleries} | {percent:3d}%] processing: contents/{gallery_root}'
        )

        pages_by_link = gather_gallery_pages(gallery_path)
        pages_by_scan = gather_media_from_gallery_tree(gallery_path)
        pages = dedupe_pages(pages_by_link + pages_by_scan)
        gen_count, reused_count = ensure_gallery_prebuilt_thumbnails(gallery_path, pages)
        thumb_generated += gen_count
        thumb_reused += reused_count
        result[gallery_path] = pages

    progress.finish()

    write_structure_js(structure)
    write_gallery_pages_js(result)

    total_galleries = len(target_gallery_paths) if incremental_mode else len(result)
    total_pages = sum(len(v) for v in result.values())
    print(f'Generated {STRUCTURE_JS_PATH}')
    print(f'Generated {GALLERY_PAGES_PATH}')
    if incremental_mode:
        print(
            f'Galleries rebuilt: {total_galleries}, total galleries in map: {len(result)}, '
            f'pages: {total_pages}'
        )
    else:
        print(f'Galleries: {total_galleries}, pages: {total_pages}')
    print(f'Gallery thumbnails: generated={thumb_generated}, reused={thumb_reused}')

    record_photo_diff(PHOTO_DIR, HISTORY_PATH)


if __name__ == '__main__':
    main()
